...
Jangan edit langsung - edit source di folder dockman_main/ lalu build ulang.
"""
'''
...
def should_skip_import(stripped: str) -> bool:
...
def extract_body(content: str) -> tuple:
    imports = []
    body    = []
    ...
    while i < len(lines):
        ...
        if is_top_level:
            if should_skip_import(stripped):
                i += 1
                continue
...
def build():
    ...
    for rel_path in SOURCE_FILES:
        ...
        imports, body = extract_body(content)
        for imp in imports:
            key = imp.strip().splitlines()[0]
    ...
    header = HEADER.format(version=version, date=datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
