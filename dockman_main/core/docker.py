...
def get_containers() -> List[Dict]:
    out, err, code = run_cmd('docker ps -a --format "{{.Names}}|{{.Status}}|{{.Image}}|{{.Ports}}"')
    if code != 0:
        return []
    containers = []
    for line in out.splitlines():
        ...
        containers.append({"name": name, "status": status, "image": image,
                           "ports": ports, "running": running, "healthy": healthy})
    return containers
...
def get_container_image(name: str) -> str:
    out, _, _ = run_cmd(f"docker inspect --format='{{{{.Config.Image}}}}' {name}")
    return out.strip()

def get_images() -> List[Dict]:
    out, _, code = run_cmd('docker images --format "{{.Repository}}:{{.Tag}}|{{.Size}}|{{.CreatedSince}}|{{.ID}}"')
    if code != 0:
        return []
    images = []
    for line in out.splitlines():
...
def get_stats_once() -> List[Dict]:
    out, _, code = run_cmd('docker stats --no-stream --format "{{.Name}}|{{.CPUPerc}}|{{.MemUsage}}|{{.NetIO}}|{{.BlockIO}}"')
    if code != 0:
        return []
    stats = []
    for line in out.splitlines():
        if not line.strip():
            continue
        parts = line.split("|")
        stats.append({"name": parts[0] if len(parts) > 0 else "?",
...
def get_screens() -> List[Dict]:
    ...
    for line in out.splitlines():
        ...
        if "." in line and any(s in line for s in ("Attached", "Detached", "Dead")):
            ...
            status = next((p.strip("()") for p in parts
                           if any(s in p for s in ("Attached", "Detached", "Dead"))), "Unknown")
            sessions.append({"sid": sid, "pid": pid, "name": name, "status": status})
    return sessions
