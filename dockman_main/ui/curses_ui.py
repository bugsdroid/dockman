"""
ui/curses_ui.py - Interactive TUI berbasis Curses untuk Dockman
"""
...
from core.docker import get_containers, get_images, get_stats_once, get_disk_usage
from core.docker import get_screens, screen_kill, container_action, get_container_logs
from core.docker import get_container_inspect, get_container_image, get_dangling_images
from core.docker import get_orphan_volumes, remove_image, compose_validate
...
def screen_home(stdscr):
    ...
    while True:
        ...
        if row < h - 4:
            ...
            safe_add(stdscr, row, 12, f"> {run} running", C_OK())
            safe_add(stdscr, row, 28, f". {stop} stopped", C_ERR())
            safe_add(stdscr, row, 44, f"* {len(images)} images", C_HEADER())
            row += 1
...
def menu_container(stdscr, c: Dict):
    ...
    if action == "logs":
        ...
        log_content = get_container_logs(name, tail=50)
        ...
        exit_tui_for_rich(stdscr)
        json_str = get_container_inspect(name)
        ...
        if confirm(stdscr, f"Restart container '{name}'?"):
            exit_tui_for_rich(stdscr)
            ok, msg = container_action(name, "restart")
        ...
        if confirm(stdscr, f"Stop container '{name}'?"):
            exit_tui_for_rich(stdscr)
            ok, msg = container_action(name, "stop")
            (rich_ui.cli_success if ok else rich_ui.cli_error)(msg)
            rich_ui.wait_key()
            restore_tui(stdscr)
        ...
        exit_tui_for_rich(stdscr)
        ok, msg = container_action(name, "start")
        (rich_ui.cli_success if ok else rich_ui.cli_error)(msg)
        rich_ui.wait_key()
        restore_tui(stdscr)
        ...
        if confirm(stdscr, f"HAPUS container '{name}'?\n(harus sudah stop)"):
            exit_tui_for_rich(stdscr)
            ok, msg = container_action(name, "rm")
            (rich_ui.cli_success if ok else rich_ui.cli_error)(msg)
            rich_ui.wait_key()
            restore_tui(stdscr)
    ...
    while True:
        ...
        if action == "list":
            ...
            if cmd:
                ...
                if ret == 0:
                    rich_ui.cli_success(f"Session '{nama}' berjalan.")
                else:
