"""
ui/rich_ui.py - Rich-based output untuk Dockman
"""
...
def sym(name: str) -> str:
    if not RICH_AVAILABLE:
        return {"ok": "[OK]", "err": "[!!]", "warn": "[!!]", "run": "[>]", "stop": "[.]"}.get(name, "*")
    return SYM.get(name, "*")

def _fallback_print(text: str):
    import re
    print(re.sub(r'\[/?[a-z0-9_ /]+\]', '', str(text)))
...
def show_containers(containers: List[Dict]):
    ...
    table = Table(
        ...
        title_style="bold cyan", highlight=True, show_lines=True,
    )
    ...
    for i, c in enumerate(containers, 1):
        ...
        table.add_row(str(i), f"{icon} {name_str}", status_str,
                      f"[dim]{c['image']}[/dim]", c["ports"] or "[dim]-[/dim]")
...
def show_images(images: List[Dict]):
    ...
    table = Table(
        ...
        border_style="cyan", title_style="bold cyan", show_lines=True,
    )
    table.add_column("#",       style="dim",  width=3, justify="right")
...
def show_stats(stats: List[Dict]):
    if not RICH_AVAILABLE:
        for s in stats:
            print(f"  {s['name']:<22} CPU:{s['cpu']:<8} MEM:{s['mem']:<20} NET:{s['net']:<20} BLK:{s['block']}")
        return
    ...
    for s in stats:
        try:
            ...
            cpu_val = 0.0
        if cpu_val > 80:
            cpu_style, cpu_icon = "bold red", sym("err")
        elif cpu_val > 50:
            cpu_style, cpu_icon = "yellow", sym("warn")
        else:
            cpu_style, cpu_icon = "green", sym("ok")
        table.add_row(
            f"{sym('run')} {s['name']}",
...
def stream_logs(name: str, tail: int = 50):
    ...
    console.print()
    console.print(Rule(
        f"{sym('log')} Live Logs: [bold cyan]{name}[/bold cyan]  [dim]Ctrl+C untuk stop[/dim]",
        style="cyan",
    ))
    console.print()
...
def generate_server_docs_with_progress(output_dir: str = None,
    ...
    with Progress(
        ...
        TaskProgressColumn(),
        TimeElapsedColumn(),
        console=console, transient=False, expand=False,
    ) as progress:
        gen   = ServerDocsGenerator(output_dir=output_dir, progress_cb=None)
        total = len(gen.SECTIONS)
        task  = progress.add_task(f"[cyan]{sym('arrow')} Memulai...[/cyan]", total=total)
